...
window.onload = (function () {
  fetch(`https://ffxivcollect.com/api/minions?language=${idioma}&limit=30`, requestOptions)
...
botonTitulo.addEventListener("click", function () {
    ...
    fetch(`https://ffxivcollect.com/api/minions?language=${idioma}&name_${idioma}_cont=${nombreTitulo.value}`, requestOptions)
    .then(response => response.json())
    .then((data) => {
      data.results.forEach((element) => {
