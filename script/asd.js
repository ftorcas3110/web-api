...
botonLogro.addEventListener("click", function () {
    ...
    fetch(`https://ffxivcollect.com/api/achievements?language=${idioma}_&name_${idioma}_cont=${nombreLogro.value}&description_${idioma}cont=${descripcionLogro.value}`, requestOptions)
    .then(response => response.json())
    .then((data) => {
      data.results.forEach((element) => {
        contenedor = document.getElementsByClassName("contenedorLogros");
